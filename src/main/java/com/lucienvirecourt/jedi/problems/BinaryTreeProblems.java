...
public class BinaryTreeProblems {
  public static <T extends Comparable<T>> boolean isSameTree(TreeNode<T> p, TreeNode<T> q) {
  ...
  private static <T extends Comparable<T>> int getHeight(TreeNode<T> root) {
    if (root == null) return 0;
  ...
  public static <T extends Comparable<T>> int diameter(TreeNode<T> root) {
    int[] max = new int[]{-(int) 1e9};
    diameter(root, max);
    return max[0];
  }

  private static <T extends Comparable<T>> int diameter(TreeNode<T> root, int[] max) {
    if (root == null) return 0;
  ...
  private static int maxPathSum(TreeNode<Integer> root, int[] max) {
    if (root == null) return 0;
  ...
  public static <T extends Comparable<T>> List<T> leftView(TreeNode<T> root) {
    List<T> result = new ArrayList<>();
    leftView(root, 0, result);
    return result;
  }

  private static <T extends Comparable<T>> void leftView(TreeNode<T> root, int level, List<T> result) {
    // Root Left Right
    if (root == null) return;
  ...
  public static TreeNode<Integer> lcs(TreeNode<Integer> root, TreeNode<Integer> p, TreeNode<Integer> q) {
  ...
  public static void childrenSumProperty(TreeNode<Integer> root) {
    if (root == null) return;
    int child = 0;
  ...
  private static void markParent(TreeNode<Integer> root, Map<TreeNode<Integer>, TreeNode<Integer>> map) {
    Deque<TreeNode<Integer>> queue = new ArrayDeque<>();
    queue.offer(root);
    while (!queue.isEmpty()) {
      TreeNode<Integer> current = queue.poll();
